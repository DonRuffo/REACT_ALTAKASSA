        ...
        <>
            {pulseTra ? <EsqueletoSoliCli />
                : (
                    <>
                        <section>
                            <h1 className="text-3xl text-center text-cyan-700 dark:text-cyan-500 font-CalSans mt-20 lg:mt-5">Solicitudes</h1>
                                ...
                                {trabajos.length !== 0 && trabajos.some(tra => tra.status === "En espera") ? (
                                    trabajos.map((tra) => (
                                        tra.status === "En espera" && (
                                            <div key={tra._id} className="w-fit h-fit max-w-64 py-3 px-5 radial-gradientTrabajos-bg rounded-lg shadow-lg shadow-cyan-300 mb-5">
                                                ...
                                                <div className="flex justify-around mt-1 gap-x-4">
                                                    <div data-tooltip-id="actualizar" data-tooltip-content={'Actualizar'} className="flex flex-col justify-center items-center py-0.5 rounded-md font-semibold text-cyan-800 hover:scale-105 transition-all duration-300 ease-in-out cursor-pointer" onClick={() => { seleccionarTrabajo(tra._id); seleccionarOferta(tra.oferta._id); setModalTraActual(!modalTraActual) }}>
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-7">
                                                            <path strokeLinecap="round" strokeLinejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182m0-4.991v4.99" />
                                                        </svg>
                                                        <span className="text-sm">Actualizar</span>
                                                    </div>
                                                    <div data-tooltip-id="eliminar" data-tooltip-content={'Eliminar'} className="flex flex-col justify-center items-center py-0.5 rounded-md text-red-800 font-semibold hover:scale-105 transition-all duration-300 ease-in-out cursor-pointer" onClick={async () => { await EliminarTrabajo(tra._id, tra.servicio) }}>
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-7">
