...
const ModalFotoProvs = ({url}) => {
    const { modalProvs, setModalProvs } = useContext(OfertaContext)
    return (
        <>
            <div className="fixed inset-0 bg-black bg-opacity-80 flex flex-col items-center justify-center overflow-auto">
                <button type="button" className="absolute top-4 right-4 text-white hover:text-red-500 duration-150" onClick={()=>setModalProvs(!modalProvs)}>
