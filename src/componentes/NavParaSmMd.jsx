...
const NavInfo = () => {
    ...
    return (
        <section className="lg:hidden fixed z-10 w-full flex justify-between mb-3 px-4 py-4 backdrop-blur-sm bg-white/30 dark:bg-black/30">
            <div className="w-2/5 text-emerald-600 flex justify-between" id="nav">
            ...
            <div className={`relative w-1/5 flex justify-center text-cyan-500 bg-gray-100 dark:bg-gray-700 rounded-2xl items-center -space-x-1 ${tipo === 'admin' ? 'hidden' : ''}`} onClick={() => setRotar(!rotar)}>
                {tipoM === 'Proveedor' ? (
                ...
                <div className={`${rotar ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'} absolute z-10 top-10 p-1 w-max rounded-md bg-gray-100 dark:bg-gray-800 outline transition-all duration-300 ease-in-out`} onClick={() => { cambioDeTipo(); setRotar(!rotar) }}>
                    <h1 className="text-cyan-500 font-semibold text-sm px-3 py-1">Cambiar a {tipoUsuario}</h1>
                </div>
