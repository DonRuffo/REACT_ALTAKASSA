...
const OpcionConfig = ({titulo, link, logo}) => {
    return (
        <li className="group/list w-full rounded-xl hover:bg-gray-100 duration-100 ">
            <Link to={link} className="flex justify-between px-5 py-3"> 
                <div className="flex gap-2">
                    <img src={logo} alt={titulo} width={25} height={20}/>
                    {titulo}
                </div> 
                <Link to={link} className="flex group/opt invisible group-hover/list:visible group-hover/list:text-slate-500">
